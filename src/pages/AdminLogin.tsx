...
export default function AdminLogin({ onNavigate }: AdminLoginProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (isActiveAdmin) {
        if (!adminDoc?.exists()) {
          await setDoc(adminRef, { active: true, email: user.email, createdAt: new Date().toISOString() }, { merge: true }).catch(() => {});
        }
        onNavigate("/admin");
      } else {
        await signOut(auth);
        setError(`Acesso recusado. A conta do Google (${user.email || ""}) não possui privilégios de administrador.`);
      }
    } catch (err: any) {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setError("Erro na autenticação com Google: " + (err.message || err.code));
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/invalid-credential" || err.code === "auth/wrong-password" || err.code === "auth/user-not-found") {
        setError("E-mail ou senha incorretos. Se ainda não possui uma conta cadastrada, clique em 'Cadastre-se' abaixo ou entre com o Google.");
      ...
      <div className="bg-card-main border border-border-main rounded-[28px] max-w-md w-full p-8 space-y-6 relative overflow-hidden shadow-2xl">
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={loading}
            className="w-full py-3 bg-card-inner hover:bg-card-elevated border border-border-main text-text-main rounded-xl font-extrabold text-xs shadow-sm transition-all flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50"
          >
          ...
          <div className="relative flex items-center justify-center">
            <div className="border-t border-border-main w-full" />
            <span className="bg-card-main px-3 text-[10px] font-extrabold text-text-muted uppercase tracking-wider relative">
              ou com e-mail
            </span>
          </div>
        </div>
