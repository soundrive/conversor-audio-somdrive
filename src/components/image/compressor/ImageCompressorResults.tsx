...
import { Download, FileArchive, CheckCircle2, RotateCcw, AlertCircle, Loader2, ArrowRight } from "lucide-react";
    ...
    try {
      ...
      const zipFiles = await Promise.all(
        completedItems.map(async (item) => {
          // If compressed is larger or equal to original, use original file buffer if preferred
          const blobToUse = (item.isLargerThanOriginal ? item.file : item.compressedBlob)!;
          const arrayBuffer = await blobToUse.arrayBuffer();
          return {
            filename: item.compressedFileName || `${item.name}-comprimido.jpg`,
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-border-main/60 pb-5">
        <div className="flex items-center gap-3.5">
          ...
          <div className="space-y-1">
            ...
            {totalSavedBytes > 0 && (
              <div className="text-xs font-black text-emerald-400 flex items-center gap-1 pt-0.5">
                <span>Economia Total: {formatBytes(totalSavedBytes)} ({totalSavedPercentage}% de redução)</span>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-card-inner/60 p-3 rounded-xl border border-border-main/50 text-[11px] space-y-1.5 font-semibold">
                ...
                {item.savedBytes && item.savedBytes > 0 ? (
                  <div className="flex justify-between items-center pt-1 border-t border-border-main/40 text-emerald-400 font-black">
                    <span>Economia:</span>
                    <span>-{formatBytes(item.savedBytes)} ({item.savedPercentage}%)</span>
                  </div>
                ) : (
                  <div className="pt-1 border-t border-border-main/40 text-amber-400 text-[10px] font-bold leading-tight">
                    Esta imagem já estava otimizada e não ficou menor com a configuração escolhida.
                  </div>
                )}
              </div>
            </div>
