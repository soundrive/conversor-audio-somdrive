...
interface ImageCompressorProgressProps {
  currentIndex: number;
  totalCount: number;
  successCount: number;
  failedCount: number;
...
export default function ImageCompressorProgress({
  currentIndex,
  totalCount,
  successCount,
  failedCount,
  onCancel
}: ImageCompressorProgressProps) {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="font-extrabold text-sm text-text-main">
              Comprimindo imagem {Math.min(currentIndex + 1, totalCount)} de {totalCount}
            </h4>
